...
public class MLPlocal implements AI {
	...
	public UserData takeNextAction() {
		...
		/*
		 * --Random-- for(int i=0; i<possibleActions; i++){ locations[i] =
		 * randomPos(); zTypes[i] = randomZone();
		 * w.setAllZonesAround(locations[i], zTypes[i], 1, true); results[i] =
		 * getOutput(getInputArrayFromWorld(w)); if(results[i]>maxScore){
		 * maxScore = results[i]; maxIndex=i; } if(results[i]<minScore){
		 * minIndex=i; minScore = results[i]; } }
		 */
